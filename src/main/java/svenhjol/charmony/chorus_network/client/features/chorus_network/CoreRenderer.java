...
import svenhjol.charmony.chorus_network.common.features.chorus_network.CoreBlockEntity;

public class CoreRenderer<T extends CoreBlockEntity> implements BlockEntityRenderer<T> {
     ...
     */
    @Override
    public void render(T core, float f, PoseStack poseStack, MultiBufferSource multiBufferSource, int i, int j, Vec3 vec3) {
        var pose = poseStack.last().pose();
        var vertexConsumer = multiBufferSource.getBuffer(RenderType.endGateway());

        var sideMinMargin = 0.26f;
        var sideMaxMargin = 0.74f;
        var vertMinMargin = 0.01f;
        var vertMaxMargin = 0.49f;
